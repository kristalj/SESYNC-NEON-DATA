...
#temp <- tempfile()
#temp2 <- ("/nfs/rswanwick-data/DASY/") # put user directory here instead of temp file -- “/nfs/rswanwick-data/myneonfiles/
...
imp.surf.desc <- raster("/nfs/rswanwick-data/DASY//NLCD_2016_Impervious_descriptor_L48_20190405.img")
...
Get_Dasy_Data(stid = "24", ctyid = "011")
...
fipscodes_mini <- fipscodes[fipscodes$stid == "24", ][1:4,]


library(rslurm)
sjob = slurm_apply(Get_Dasy_Data, fipscodes_mini[c(2,4),], jobname = 'DASYtest',
                    nodes = 1, cpus_per_node = 2, pkgs = c("tidycensus", "raster", "tidyverse", "FedData", "sf", "dplyr", "glue"),
                    submit = TRUE)
