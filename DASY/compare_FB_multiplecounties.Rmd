...
Copy the tifs from Rachel S directory to QDR directory due to ongoing issue with permissions.
...
tifpath="/nfs/qread-data/DASY/countyrasters"
{
  ...
  cp /nfs/rswanwick-data/DASY/tifs/neon-dasy-${statefips}-${countyfips}.tif /tmp
  done
} < /tmp/countycodesforfb.csv
```
...
tifpath="/nfs/qread-data/DASY/countyrasters"
{
  ...
  while IFS=, read -r county state fips; do 
    ...
    gdalwarp /tmp/neon-dasy-${statefips}-${countyfips}.tif ${tifpath}/county${fips}_dasy.tif \
...
It's sort of hard to tell what is going on but I think there is sort of a pattern where the bigger the county is, the worse the discepancy.
