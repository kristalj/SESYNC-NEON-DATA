...
The following code block is a set of Bash shell commands to mosaic the county-level GeoTiffs together as a virtual raster with `gdalbuildvrt`,
and as a GeoTiff with `gdal_merge.py`.

```{bash, eval = FALSE}
cd DASY/tifs
gdalbuildvrt dasy_conus_2021-08-12.vrt *.tif
gdal_merge.py -o dasy_conus_2021-08-12.tif -of GTiff -co COMPRESS=LZW -co BIGTIFF=YES -n -3.39999999999999996e+38 neon-dasy-*.tif
```
