...
We masked out all impervious surface pixels with a value less than or equal to 0.01 (less than 1% of the pixel's surface area is impervious).
Census blocks with zero population in the 2010 decennial census were masked out completely regardless of what impervious surfaces they contained.
Using the impervious surface descriptor, we masked out all pixels classified as primary, secondary, and tertiary urban roads.
...
- Clips 2016 NLCD impervious percentage raster to the county area
- Masks out pixels with <1% impervious surface, pixels in blocks with zero population in 2010, and pixels identified as roads
...
Get_Dasy_Data <- function(stid, ctyid) {
  ...
  # Use gdalwarp to extract the county area, from the NLCD impervious raster, already in Albers projection
  ...
  # impervious surface descriptor dataset 
...
## Correct missing data values

After the GeoTIFFs were generated, we manually replaced the `nodata` values in each GeoTIFF with `-9999` so that they can be merged into a single large GeoTIFF.

```{bash, eval = FALSE}
mkdir DASY/tifs_corrected
infiles=`cd DASY/tifs && echo neon-dasy-*.tif`
for tiffile in $infiles; do
  gdalwarp -dstnodata -9999 -co COMPRESS=LZW DASY/tifs/$tiffile DASY/tifs_corrected/$tiffile
done
```
...
```{bash, eval = FALSE}
cd DASY/tifs_corrected
gdalbuildvrt dasy_conus_2021-08-17.vrt *.tif
gdal_translate -co COMPRESS=LZW -co BIGTIFF=YES -of GTiff dasy_conus_2021-08-17.vrt dasy_conus_2021-08-17.tif
```
