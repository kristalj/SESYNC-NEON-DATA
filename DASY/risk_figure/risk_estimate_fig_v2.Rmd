...
flood_rasters <- map(counties_flood$wse_filename, raster::raster)
...
We will use `st_as_sf()` to convert the wildfire raster and flood raster (converted to binary) to polygons. 
...
flood_polygons_ourdasycrs <- map(wse_notna_poly, st_transform, crs = st_crs(ourdasy_rasters[[1]]))
flood_polygons_epadasycrs <- map(wse_notna_poly, st_transform, crs = st_crs(epadasy_rasters[[1]]))
flood_polygons_huangcrs <- map(wse_notna_poly, st_transform, crs = st_crs(huang_rasters[[1]]))
flood_polygons_fbpopcrs <- map(wse_notna_poly, st_transform, crs = st_crs(fbpop_rasters[[1]])) %>%
  map(st_make_valid)
